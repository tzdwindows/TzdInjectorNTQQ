...
chii.start({ port }, () => {
    console.log(`TzdInjectorNTQQ DevTools server running on port ${port}`);
});

ipcMain.handle("tzdwindows_7.chii_devtools.ready", () => port);

// 在创建浏览器窗口后添加以下代码
function connectToChii(webContents) {
    return new Promise((resolve) => {
        ...
        const injectScript = `
            (function() {
                ...
                script.onload = () => console.log('[TzdInjectorNTQQ] 客户端已注入');
                document.head.appendChild(script);
            })();
        `;

        webContents.executeJavaScript(injectScript).catch(console.error);

        // 监听客户端连接
        webContents.on('console-message', (_, level, message) => {
            if (message.includes('[TzdInjectorNTQQ] 客户端已注入')) {
                console.log('[TzdInjectorNTQQ] 目标页面已连接到调试服务器');
                resolve();
            }
        });
    });
...
async function openDevTools(window) {
    console.log('[TzdInjectorNTQQ] 正在打开DevTools窗口');

    try {
        // 1. 确保窗口已连接到 Chii
        await connectToChii(window.webContents);

        // 2. 获取目标列表（增加重试次数和间隔）
        let targetsData = null;
        for (let attempt = 1; attempt <= 15; attempt++) {
            console.log(`[TzdInjectorNTQQ] 尝试 #${attempt}: 获取目标列表`);

            try {
                const response = await fetch(`http://localhost:${port}/targets`);
                if (!response.ok) throw new Error(`HTTP ${response.status}`);

                targetsData = await response.json();
                console.log(`[TzdInjectorNTQQ] 目标数量: ${targetsData.targets?.length || 0}`);

                if (targetsData.targets?.length > 0) break;

                // 逐渐增加等待时间
                const waitTime = Math.min(attempt * 300, 2000);
                await new Promise(r => setTimeout(r, waitTime));
            } catch (error) {
                console.warn(`[TzdInjectorNTQQ] 尝试 #${attempt} 失败:`, error.message);
        ...
        console.log('[TzdInjectorNTQQ] 当前窗口URL:', currentUrl);
        ...
        console.log('[TzdInjectorNTQQ] 标准化URL:', currentNormalized);

        // 4. 查找匹配目标
        let targetFound = null;
        for (const target of targetsData.targets) {
            const targetNormalized = normalizeUrl(target.url);
            console.log(`[TzdInjectorNTQQ] 检查目标: ${targetNormalized}`);

            // 宽松匹配：比较标准化后的路径
            if (currentNormalized === targetNormalized) {
                targetFound = target;
                console.log(`[TzdInjectorNTQQ] 精确匹配目标: ${target.id}`);
                break;
            }

            // 部分匹配：检查是否包含主要路径
            const currentParts = currentNormalized.split('/').filter(p => p);
            const targetParts = targetNormalized.split('/').filter(p => p);

            if (currentParts.length > 0 &&
                targetParts.length > 0 &&
                currentParts[currentParts.length - 1] === targetParts[targetParts.length - 1]) {
                targetFound = target;
                console.log(`[TzdInjectorNTQQ] 部分匹配目标: ${target.id}`);
                break;
            }
        }

        if (!targetFound) {
            console.error('[TzdInjectorNTQQ] 所有可用目标:', JSON.stringify(targetsData.targets, null, 2));
            throw new Error(`未找到匹配目标\n当前URL: ${currentNormalized}`);
        }

        // 5. 创建调试窗口
        const devtoolsUrl = `http://localhost:${port}/front_end/chii_app.html` +
            `?ws=localhost:${port}/client/TzdInjectorNTQQ?target=${targetFound.id}`;

        console.log(`[TzdInjectorNTQQ] 打开开发者工具: ${devtoolsUrl}`);

        const devtoolsWindow = new BrowserWindow({
            width: 1200,
            height: 800,
            show: true,
            title: `开发者工具 - ${targetFound.url}`,
            webPreferences: {
                devTools: false,
        ...
        devtoolsWindow.on('ready-to-show', () => {
            devtoolsWindow.show();
            console.log('[TzdInjectorNTQQ] 开发者工具窗口已显示');
        });

        // 处理加载失败
        devtoolsWindow.webContents.on('did-fail-load', (_, code, desc) => {
            console.error(`[TzdInjectorNTQQ] 加载开发者工具失败: ${code} ${desc}`);
        });

        await devtoolsWindow.loadURL(devtoolsUrl);
        console.log('[TzdInjectorNTQQ] 开发者工具加载完成');

        return devtoolsWindow;

    } catch (error) {
        console.error("[TzdInjectorNTQQ] 打开开发者工具失败:", error);
        if (dialog) {
            dialog.showErrorBox(
                "开发者工具错误",
                `无法打开开发者工具:\n${error.message}\n\n查看控制台获取详细信息`
    ...
    app.on('browser-window-created', (_, win) => {
        ...
        win.webContents.on("before-input-event", async (event, input) => {
            if (input.key == "F12" && input.type == "keyUp") {
                console.log('[chii] chii已启动')
                if (devtools_window) {
                    devtools_window.close();
                    devtools_window = null;
                }
                else {
                    devtools_window = await openDevTools(win);
